	...
	if (IsLocalPlayerController())
	{
		if (SVirtualJoystick::ShouldDisplayTouchInterface())
		{
			...
			if (MobileControlsWidget)
			{
				...
				UE_LOG(LogroboExp, Error, TEXT("Could not spawn mobile controls widget."));
		...
		if (UIWidget)
		{
			...
			UE_LOG(LogroboExp, Error, TEXT("Could not spawn Time Trial UI widget."));

		}
		

		// spawn the UI widget and add it to the viewport
		VehicleUI = CreateWidget<UroboExpUI>(this, VehicleUIClass);
		...
		if (VehicleUI)
		{
			...
			UE_LOG(LogroboExp, Error, TEXT("Could not spawn vehicle UI widget."));
...
void ATimeTrialPlayerController::OnPossess(APawn* InPawn)
{
	...
	VehiclePawn = CastChecked<AroboExpPawn>(InPawn);
...
void ATimeTrialPlayerController::OnPawnDestroyed(AActor* DestroyedPawn)
{
	...
	if (ActorList.Num() > 0)
	{
		...
		if (AroboExpPawn* RespawnedVehicle = GetWorld()->SpawnActor<AroboExpPawn>(VehiclePawnClass, SpawnTransform))
		{
			// possess the vehicle
